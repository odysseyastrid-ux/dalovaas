...
// Animated mesh-gradient footer background — a small hand-written WebGL
// shader (no library) blending 4 drifting color anchors, warped by two
// sine wave fields and rotated, approximating the "mesh gradient" look
// requested for the footer. Guarded on the canvas existing since only
// index.html has one.
(function initFooterMeshGradient(){
  const canvas = document.getElementById('footerMeshCanvas');
  if (!canvas) return;
  const gl = canvas.getContext('webgl') || canvas.getContext('experimental-webgl');
  if (!gl) return; // CSS gives the footer a plain background either way.
  ...
  const FRAG_SRC = `
    ...
    uniform float uWaveXShift;
    uniform float uWaveY;
    uniform float uWaveYShift;
    ...
    void main(){
      ...
      float t = uTime * 0.08;
      float wx = sin(uv.y * 3.0 * uWaveX + t + uWaveXShift * 6.2831) * 0.25 * uWaveX;
      float wy = cos(uv.x * 3.0 * uWaveY + t * 1.3 + uWaveYShift * 6.2831) * 0.25 * uWaveY;
      vec2 wuv = uv + vec2(wx, wy);

      vec2 p0 = vec2(-0.55, -0.5) + 0.18 * vec2(sin(t * 0.7), cos(t * 0.6));
      vec2 p1 = vec2( 0.55, -0.4) + 0.18 * vec2(cos(t * 0.5), sin(t * 0.8));
      vec2 p2 = vec2(-0.45,  0.5) + 0.18 * vec2(sin(t * 0.9 + 2.0), cos(t * 0.4));
      vec2 p3 = vec2( 0.5,   0.5) + 0.18 * vec2(cos(t * 0.6 + 1.0), sin(t * 0.7 + 1.0));

      float d0 = length(wuv - p0);
      float d1 = length(wuv - p1);
      float d2 = length(wuv - p2);
      float d3 = length(wuv - p3);

      // Higher "mixing" = softer, more blended transitions between the
      // four colors (lower inverse-distance exponent).
      float sharpness = mix(6.0, 1.2, clamp(uMixing, 0.0, 1.0));
      float w0 = 1.0 / pow(d0 + 0.001, sharpness);
      float w1 = 1.0 / pow(d1 + 0.001, sharpness);
      float w2 = 1.0 / pow(d2 + 0.001, sharpness);
      float w3 = 1.0 / pow(d3 + 0.001, sharpness);
      float sum = w0 + w1 + w2 + w3;

      vec3 color = (uColor0 * w0 + uColor1 * w1 + uColor2 * w2 + uColor3 * w3) / sum;
  ...
  const COLORS = ['#ffad0a', '#6200ff', '#e2a3ff', '#ff99fd'].map(hexToRgb01);
  const uniforms = {
    ...
    uWaveXShift: gl.getUniformLocation(program, 'uWaveXShift'),
    uWaveY: gl.getUniformLocation(program, 'uWaveY'),
    uWaveYShift: gl.getUniformLocation(program, 'uWaveYShift'),
    uMixing: gl.getUniformLocation(program, 'uMixing'),
    uRotation: gl.getUniformLocation(program, 'uRotation'),
  };
  gl.uniform3fv(uniforms.uColor0, COLORS[0]);
  gl.uniform3fv(uniforms.uColor1, COLORS[1]);
  gl.uniform3fv(uniforms.uColor2, COLORS[2]);
  gl.uniform3fv(uniforms.uColor3, COLORS[3]);
  gl.uniform1f(uniforms.uWaveX, 1);
  gl.uniform1f(uniforms.uWaveXShift, 0.6);
  gl.uniform1f(uniforms.uWaveY, 1);
  gl.uniform1f(uniforms.uWaveYShift, 0.21);
  gl.uniform1f(uniforms.uMixing, 0.93);
  gl.uniform1f(uniforms.uRotation, (270 * Math.PI) / 180);
  ...
  function draw(now){
    resize();
    gl.uniform1f(uniforms.uTime, reduceMotion ? 0 : now * 0.001);
    gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);
    if (!reduceMotion) rafId2 = requestAnimationFrame(draw);
  }
  gl.uniform1f(uniforms.uTime, 0);
  gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4);

  // Only actually animates while the footer is on screen — no point
  // spending frames on a canvas nobody can see.
  const io = new IntersectionObserver((entries) => {
    const visible = entries[0].isIntersecting;
    if (visible && rafId2 === null && !reduceMotion) rafId2 = requestAnimationFrame(draw);
    else if (!visible && rafId2 !== null) { cancelAnimationFrame(rafId2); rafId2 = null; }
  }, { threshold: 0 });
  io.observe(canvas);
})();
