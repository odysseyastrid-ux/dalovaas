-- Loyalty points move from "25 pts per 1500 FCFA spent" to a flat 14.5 pts
-- per item ordered (regardless of price), per the owner's new program.
...
begin
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    v_total_qty := v_total_qty + greatest(1, (v_line->>'qty')::integer);
...
-- The "Free Mugi Burger" reward (100 pts) still pointed at the prototype's
-- old seed item ("The Classic Sanj", 3500 FCFA) instead of the real MUGI
-- BURGER (1450 FCFA) added later -- fix the link so the reward actually
-- gives what its name says.
update rewards set item_id = 'itm_2a51058fb4', name = 'Free Mugi Burger', name_fr = 'Mugi Burger offert'
  where id = 'r2';

-- New 100-point reward: any free soft drink, as an alternative to the burger.
insert into rewards (id, name, name_fr, cost, item_id, active)
values ('r4', 'Free Drink', 'Boisson offerte', 100, 'd1', true)
on conflict (id) do update set name = excluded.name, name_fr = excluded.name_fr, cost = excluded.cost, item_id = excluded.item_id, active = excluded.active;

-- Poutine promo: 20% off (2900 -> 2320 FCFA), backing the new promo banner.
update menu_items set price = 2320 where id = 'f2' and cat = 'poutine';
