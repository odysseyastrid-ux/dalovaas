-- Combo upsell: a burger line flagged combo=true gets frites + a chosen
-- drink synthesized as extra order lines, a poutine line just gets the
-- drink. The bundle (item + fries[burger only] + drink) is discounted 15%,
-- same as this project's other discounts: a lump sum subtracted from the
-- order subtotal, never trusted from the client.
...
declare
  ...
  v_order orders;
  v_drink_item menu_items;
  v_fries_item menu_items;
  v_combo_total integer;
  v_combo_discount integer;
begin
  ...
  for v_line in select * from jsonb_array_elements(p_lines) loop
    ...
    if coalesce((v_line->>'combo')::boolean, false) then
      if v_item.cat not in ('burgers', 'poutine') then
        raise exception 'combo not available for %', v_item.name;
      end if;

      select * into v_drink_item from menu_items
        where id = (v_line->>'combo_drink_id') and cat = 'soft_drinks' and not deleted and not out_of_stock;
      if v_drink_item is null then
        raise exception 'combo drink not available';
      end if;

      v_combo_total := v_line_total + v_drink_item.price * v_qty;
      v_lines_out := v_lines_out || jsonb_build_array(jsonb_build_object(
        'name', v_drink_item.name,
        'cat', v_drink_item.cat,
        'qty', v_qty,
        'unitPrice', v_drink_item.price,
        'addOns', '[]'::jsonb,
        'lineTotal', v_drink_item.price * v_qty
      ));
      v_subtotal := v_subtotal + v_drink_item.price * v_qty;
      v_total_qty := v_total_qty + v_qty;

      if v_item.cat = 'burgers' then
        select * into v_fries_item from menu_items
          where cat = 'fries' and not deleted and not out_of_stock
          order by sort_order limit 1;
        if v_fries_item is null then
          raise exception 'combo fries not available';
        end if;
        v_lines_out := v_lines_out || jsonb_build_array(jsonb_build_object(
          'name', v_fries_item.name,
          'cat', v_fries_item.cat,
          'qty', v_qty,
          'unitPrice', v_fries_item.price,
          'addOns', '[]'::jsonb,
          'lineTotal', v_fries_item.price * v_qty
        ));
        v_subtotal := v_subtotal + v_fries_item.price * v_qty;
        v_total_qty := v_total_qty + v_qty;
        v_combo_total := v_combo_total + v_fries_item.price * v_qty;
      end if;

      v_combo_discount := round(v_combo_total * 15 / 100.0);
      v_discount := v_discount + v_combo_discount;
    end if;
  end loop;
  ...
  if p_promo_code is not null then
    ...
    if v_percent_off is not null then
      v_discount := v_discount + round(v_subtotal * v_percent_off / 100.0);
    else
      p_promo_code := null;
    end if;
  end if;
  ...
  update accounts set loyalty_points = loyalty_points + round(v_total_qty * 14.5)
    where id = auth.uid();
